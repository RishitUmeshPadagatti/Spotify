...
async function getAndPopulateFolders() {
    ...
    let temp = await fetch(`https://rishit.freewebhostmost.com/songs/`)
...
async function refreshSongsInAPlayist(folderName, songsPathArray) {
    ...
    for (const songRelativePath of songsPathArray) {
        let metaTags = await getMetaTags(`https://rishit.freewebhostmost.com/${songRelativePath}`);
...
async function getGlobalSongs() {
    globalSongs = []
    let temp = await fetch(`https://rishit.freewebhostmost.com/songs/global/`)
    let response = await temp.text()
    let tempDiv = document.createElement("div")
    tempDiv.innerHTML = response
    let as = tempDiv.getElementsByTagName("a")
...
async function setPreliminaryControlsAndStuff() {
    ...
    if (LSlastPlayedSong.endsWith(".mp3") == true) {
        let metaTags = await getMetaTags(`https://rishit.freewebhostmost.com/songs/global/${LSlastPlayedSong}`)
...
async function setUpQueueHtml() {
    for (const ulQueueSongHtml of document.querySelectorAll(".ulSongQueueHtml")) {
        ...
        for (const queueSong of queue) {
            let metaTags = await getMetaTags(`https://rishit.freewebhostmost.com/songs/global/${queueSong}`)
            let likePath = "icons/like.svg"
...
async function previousSong() {
    ...
    if (index != 0) {
        ...
        let tempMetaTags = await getMetaTags(`https://rishit.freewebhostmost.com/songs/global/${previousSong}`)
...
async function nextSong() {
    ...
    if (LSrepeat == 0 || LSrepeat == 1) {
        ...
        let tempMetaTags = await getMetaTags(`https://rishit.freewebhostmost.com/songs/global/${nextSong}`)
...
async function playPlaylist() {
    ...
    if (currentlyOpenedPlaylist == currentFolder) {
        ...
        let metaTags = await getMetaTags(`https://rishit.freewebhostmost.com/songs/global/${firstSong}`)
